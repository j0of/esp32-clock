...
std::vector<Alarm> alarms = {
  Alarm{ .weekends = false, .hour = 7, .name = "WAKE UP WAKE UP WAKE UP" },
  Alarm{ .weekends = true, .hour = 22, .minute = 30, .name = "Bedtime" }
};
...
void updateTime() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("Failed to update time: no connection");
    return;
  }
...
void checkAlarms() {
  for (auto &alarm : alarms) {
    ...
    if (alarm.hour != hour || alarm.minute != minute)
      continue;
    if (!alarm.weekends && (dayOfWeek == "Sunday" || dayOfWeek == "Saturday"))
      continue;
    currentAlarm = &alarm;
    resetDisplayAlarm();
  }
